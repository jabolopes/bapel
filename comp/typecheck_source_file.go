...
func newContext() (stlc.Context, error) {
	...
	binds := []stlc.Bind{
		...
		stlc.NewTermDeclBind("ifthen",
			ir.ForallVars([]ir.TypeParam{ir.TypeParam{Var: "a", Kind: ir.NewTypeKind()}},
				ir.NewFunctionType(
...
func (c *sourceFileChecker) addTraitImpl(impl *ir.IrTraitImpl) error {
	...
	for _, tp := range impl.TypeParams {
		ctx, err = ctx.AddBind(stlc.NewTypeParamBind(tp))
		if err != nil {
			return err
		}
	}
	...
	if impl.Case == ir.InherentImpl {
		...
		for _, m := range impl.Methods {
			...
			tvars = append(tvars, m.TypeParams...)
			if len(tvars) > 0 {
...
func (c *sourceFileChecker) checkTraitImpl(impl *ir.IrTraitImpl) error {
	...
	var err error
	for _, tp := range impl.TypeParams {
		methodContext, err = methodContext.AddBind(stlc.NewTypeParamBind(tp))
		if err != nil {
			return err
		}
	}

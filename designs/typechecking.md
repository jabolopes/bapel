...
*   **Location:** [ts/stlc/inferencer.go](ts/stlc/inferencer.go) (invoked via [InferFunction](ts/stlc/typechecker.go#L29-L38)).
*   **Role:** The inferencer is responsible for all complex, heuristic-heavy type analysis and program transformations:
    ...
    *   Method resolution and trait lookup ([LookupMethod](ts/stlc/context.go#L244)).
...
*   **Location:** [ts/stlc/typechecker_typecheck.go](ts/stlc/typechecker_typecheck.go) (invoked via [TypecheckFunction](ts/stlc/typechecker.go#L47-L103)).
...
*   **Location:** Handled in a separate pass before inference in [ast/desugar.go](ast/desugar.go).
...
*   **Location:** Interleaved directly inside the inferencer ([ts/stlc/inferencer.go](ts/stlc/inferencer.go)).
...
2.  **Existential Resolution ("Zonking"):** During inference, types contain temporary unification variables (`evars`). At the very end of [InferFunction](ts/stlc/typechecker.go#L29-L38), a final substitution pass (zonking) must replace all unresolved `evars` in `.Type` annotations with their solved concrete types.
3.  **Explicit Desugaring & Elaboration:** All implicit behaviors and code-generation metadata must be explicitly written into the IR during Stage 1 inference and zonking ([ts/stlc/solve_term.go](ts/stlc/solve_term.go)):
    *   **Method Calls:** Rewritten to explicit fully qualified function applications (`Type::method(adjusted_s, args)`).
    *   **Generic Instantiations:** When calling a polymorphic function `f` without explicit type arguments, the inferencer explicitly wraps it in an [AppTypeTerm](ir/ir_term.go#L540) (`f [T]`), ensuring the simple typechecker never has to guess generic type parameters.
...
4.  **Elimination of Duplication:** With this architecture established, duplicated reduction and subtyping logic in [ts/stlc/typechecker.go](ts/stlc/typechecker.go) has been removed, stripping the typechecker down to a minimal, trusted verifier.
...
1.  **Strictly Read-Only Stage 2 (`Typechecker`):**
    *   All verification methods in [ts/stlc/typechecker_typecheck.go](ts/stlc/typechecker_typecheck.go) treat the AST as read-only. There are zero mutations or assignments to `term.Type`, `c.ReducedType`, `c.TagIndex`, or `arm.Index` during verification.
...
3.  **Complete Elaboration in Stage 1:**
    *   [ts/stlc/inferencer.go](ts/stlc/inferencer.go) and [ts/stlc/solve_term.go](ts/stlc/solve_term.go) reliably populate 100% of `.Type` fields across all AST expressions, as well as code-generation metadata (`TagIndex` unconditionally on variant injections, `Index` on match arms, and `ReducedType` on projections and set terms) before handing the IR over to `TypecheckFunction`.
4.  **Minimal Kernel:**
    *   All redundant type calculation, synthesis, and unused helper methods (such as `predicateType`) have been stripped out of [ts/stlc/typechecker.go](ts/stlc/typechecker.go) and `typechecker_typecheck.go`, leaving a minimal, trusted verifier.
